//! Mosaic TUI — v0.2-alpha entry point.
//!
//! Usage:
//!   mosaic              — open most recent run
//!   mosaic `<run-id>`   — open specific run by UUID string or directory name
//!   mosaic list         — print table of recent runs and exit
//!   mosaic --help
//!   mosaic --version
...
#[command(
    name = "mosaic",
    version,
    about = "Mosaic TUI — observe Agent Shire runs",
    long_about = None,
)]
struct Cli {
    #[command(subcommand)]
    ...
    if !base.exists() {
        bail!(
            "No shire runs directory found at {}.\n\
             Run `shire` first to create a run.",
            base.display()
        );
    }
...
fn build_one_shot_snapshot(run_dir: &std::path::Path) -> crate::state::AppSnapshot {
    ...
    if let Ok(bytes) = std::fs::read(run_dir.join("summary.json")) {
        if let Ok(sum) = serde_json::from_slice::<mosaic_core::store::RunSummary>(&bytes) {
