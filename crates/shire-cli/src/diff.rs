//! `shire diff <run-a> <run-b>` — compare two prior runs side-by-side.
...
pub fn load_summary(run_dir: &Path) -> Result<RunSummary> {
        ...
        .map_while(std::io::Result::ok)
    {
        ...
        if let Ok(rec) = serde_json::from_str::<mosaic_core::store::TaskRecord>(trimmed) {
            tasks.push(rec);
        }
    }
    ...
    let tasks_failed = tasks
        .iter()
        .filter(|t| !matches!(t.status, mosaic_core::store::TaskStatus::Success))
        .count();
    ...
    let meta: mosaic_core::store::RunMeta = serde_json::from_slice(&meta_bytes)
...
fn task_metrics(rec: &mosaic_core::store::TaskRecord, model: Option<&str>) -> TaskMetrics {
...
pub fn build_report(
    ...
    let a_map: HashMap<&str, &mosaic_core::store::TaskRecord> =
        a.tasks.iter().map(|t| (t.task_id.as_str(), t)).collect();
    let b_map: HashMap<&str, &mosaic_core::store::TaskRecord> =
...
mod tests {
    ...
    fn load_summary_falls_back_to_jsonl() {
        ...
        let meta = mosaic_core::store::RunMeta {
            run_id,
            manifest_path: PathBuf::from("/tmp/test.toml"),
            shire_version: "0.1.0".to_string(),
            claude_version: None,
