...
pub async fn spawn_sublead(
    ...
    if let ResolvedEnvelope::Owned { budget_usd, .. } = &envelope {
        *state.root.reserved_usd.lock().await += *budget_usd;
    }
    ...
    for (k, v) in &req.initial_ref {
        ...
        let bytes =
            serde_json::to_vec(v).with_context(|| format!("serializing initial_ref key {k}"))?;
        sub_store
