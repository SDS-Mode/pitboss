...
fn init_tracing(verbose: u8, quiet: bool) {
    ...
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| EnvFilter::new(format!("pitboss_cli={level},pitboss_core={level}")));
    fmt()
...
fn run_diff(run_a: &str, run_b: &str, json: bool) -> ! {
    ...
    let summary_a = match diff::load_summary(&dir_a) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("pitboss diff: load run A: {e:#}");
            std::process::exit(1);
        }
    };
    let summary_b = match diff::load_summary(&dir_b) {
        Ok(s) => s,
        Err(e) => {
            eprintln!("pitboss diff: load run B: {e:#}");
            std::process::exit(1);
        }
    };
    ...
    if json {
        match serde_json::to_string_pretty(&report) {
            Ok(s) => println!("{s}"),
            Err(e) => {
                eprintln!("pitboss diff: JSON serialization: {e}");
                std::process::exit(1);
            }
        }
    } else {
