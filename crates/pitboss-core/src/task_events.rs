//! Per-worker / per-actor `events.jsonl` writer + the `TaskEvent`
//! enum that backs both the per-actor file and the run-wide
//! `audit.jsonl` tee (#414). Not loaded by the dispatcher; a post-hoc
//! audit trail only.
//!
