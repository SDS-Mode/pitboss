...
//! `TaskEvent` emission (see [`crate::task_events::append_event`]).
...
//! status`).
//!
//! Lives in `pitboss-core` (relocated under F-ARCH-5 / #484) so the web
//! console can read the run-wide audit log without depending on
//! `pitboss-cli`. The `pitboss-cli::audit` module is a re-export shim.
...
/// `task_events::AuditEntry`: `actor_id` is the run-wide
...
fn event_reason_kind(event: &TaskEvent) -> Option<&'static str> {
    match event {
        TaskEvent::ToolDenied { reason_kind, .. } => Some(match reason_kind {
            crate::task_events::DeniedReasonKind::DeniedByRule => "denied_by_rule",
            crate::task_events::DeniedReasonKind::DeniedByPolicy => "denied_by_policy",
            crate::task_events::DeniedReasonKind::DeniedByProfile => "denied_by_profile",
            crate::task_events::DeniedReasonKind::DeniedByMcpServerAllowlist => {
                "denied_by_mcp_server_allowlist"
            }
            crate::task_events::DeniedReasonKind::OperatorRejected => "operator_rejected",
            crate::task_events::DeniedReasonKind::TtlExpired => "ttl_expired",
        }),
        _ => None,
    }
}
...
mod tests {
    use super::*;
    use crate::task_events::DeniedReasonKind;
    use std::io::Cursor;
