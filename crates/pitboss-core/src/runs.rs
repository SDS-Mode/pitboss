...
//!
//! Consumed by the `pitboss-cli` (`pitboss list`/`prune`/etc.),
//! `pitboss-tui` (run picker), and `pitboss-web` (run-index DTO). All
//! three reach this module via `pitboss_core::runs` — the legacy
//! `pitboss_cli::runs::*` re-export shim is kept for source compat with
//! older imports but new code should reach into the canonical home
//! here (#484).
//!
//! ## State machine
//!
...
pub fn resolve_run_dir_by_prefix(base: &Path, id_or_prefix: &str) -> Result<PathBuf> {
    ...
    match matches.len() {
        ...
        n => bail!("{n} runs match prefix '{id_or_prefix}' — be more specific"),
    }
}
...
pub fn collect_run_entry(run_dir: &Path, run_id: String, mtime: SystemTime) -> RunEntry {
    ...
    let summary_json_state = match std::fs::read(&summary_json) {
        Ok(bytes) => match serde_json::from_slice::<crate::store::RunSummary>(&bytes) {
            Ok(s) => {
    ...
    let snap = crate::store::read_run_snapshot(run_dir);
...
pub fn control_socket_is_live(run_id: &str, run_dir: &Path) -> bool {
    ...
    let path = resolve_socket_path(run_id, run_dir);
    if !path.exists() {
        return false;
    }
...
fn resolve_socket_path(run_id: &str, run_dir: &Path) -> PathBuf {
...
fn jsonl_recent(jsonl: &Path) -> bool {
    ...
    SystemTime::now()
        ...
        .map_or(true, |elapsed| elapsed <= STALENESS_THRESHOLD)
}
...
mod tests {
    ...
    fn task_line(task_id: &str, status: crate::store::TaskStatus) -> String {
        let t = chrono::Utc::now();
        let rec = crate::store::TaskRecord {
    ...
    fn collect_run_entry_with_old_jsonl_no_socket_is_stale() {
        ...
        // socket exists.
        let jsonl = run_dir.join("summary.jsonl");
        let line = task_line("t", crate::store::TaskStatus::Failed);
    ...
    fn resolve_socket_path_does_not_double_nest_run_id() {
        ...
        let got = resolve_socket_path("run-uuid-1", &run_dir);
