...
- **Unblocked Homebrew formula push to the tap repo.** The cargo-dist
  0.28.7-generated `.github/workflows/release.yml` had
  `persist-credentials: false` on the `actions/checkout@v4` step for
  the tap repo (`publish-homebrew-formula` job). That flag tells the
  checkout action *not* to save the passed token into git config, so
  the subsequent `git push` had no credential and fell through to
  interactive prompting (which fails in CI with "could not read
  Username for 'https://github.com'", exit 128). Removed the
  `persist-credentials: false` line so the tap PAT is persisted for
  the push. v0.5.3 built and released to GitHub Releases successfully
  but never reached the tap; v0.5.4 is the first version the
  `SDS-Mode/homebrew-pitboss` tap holds. Note: this is a divergence
  from cargo-dist's autogenerated workflow — a comment marks it in
  the workflow file for future regeneration safety.
